...
class adminController extends Controller
{
    ...
    public function customerList()
    {
        ...
        foreach ($data as $record) {
            ...
            $record->balance  = ($this->getBalance($record->email) ?? 0);
        }
    ...
    private function getBalance($email)
    {
        ...
        $customer_id_in_user = User::where('email', $email)->select('id')->first();
        if (is_null($customer_id_in_user)) {
            return 0;
        }
        $customer_tabs = Tabinfo::with('user', 'passenger', 'ledger', 'tabtype')
            ->where('status_id', '=', 6)
            ->where('user_id', '=', $customer_id_in_user->id)
            ->get();
    ...
    public function TabRequestSubmitted($id)
    {
    ...
    public function TabRequestPosted($id)
    {
        $user = User::find(Auth::user()->id);
        if ($user->hasRole('Super Admin') || $user->hasPermissionTo('Posted')) {
            ...
            $customer->balance = $customer->temp_balance;
            $customer->update();
            $tabinfo->update();
    ...
    public function TabRequestProcessing($id)
    {
        $user = User::find(Auth::user()->id);
    ...
    public function TabRequestCompleted($id)
    {
        $user = User::find(Auth::user()->id);
    ...
    public function TabRequestRejected($id)
    {
        $user = User::find(Auth::user()->id);
        ...
        if ($user->hasRole('Super Admin') || $user->hasPermissionTo('Posted')) {
            ...
            if ($customer) {
                ...
                $passenger = [
                    ...
                    'tabinfo_id' => $tabinfo->id
                ];
                $dummyPassenger = Passenger::create($passenger);
                ...
                $pay_history->passenger_id = $dummyPassenger->id;
    ...
    public function delete_record($id)
    {
        ...
        if ($user->hasRole('Super Admin')) {
            ...
            //     dd($passenger->paymentHistory->balance);
            // }
            //dd($tab->tabtype->id,$tab->passenger,$customer_tab->balance,$customer_tab->credit_limit);
            $total_balance = 0;
            $total_creditlimit = 0;
            if ($tab->tabtype->id === 2 || $tab->tabtype->id === 3 || $tab->tabtype->id === 8 || $tab->tabtype->id === 16) {
                foreach ($tab->passenger as $passenger) {
                    # code...
                    $total_balance  = (int)$customer_tab->balance - (int)$passenger->price->value;
                    //    $total_creditlimit = (int)$customer_tab->credit_limit - (int)$passenger->price->value;
                    $passenger->delete();
                }
                $customer_tab->balance = $total_balance;
                //$customer_tab->credit_limit = $total_creditlimit;
                $customer_tab->save();
            }
            if ($tab->tabtype->id === 9) {
                foreach ($tab->passenger as $passenger) {
        ...
        foreach ($data as $record) {
            ...
            if ($tabinfo) {
                ...
                if ($tabinfo->tabtype_id == 2 || $tabinfo->tabtype_id == 3) {
                    (int)$pay_history->credit = (int)$price->value;
                    //(int)$customer->credit_limit = (int)($customer->credit_limit + (int)$price->value);
                    (int)$customer->temp_balance = (int)($customer->temp_balance + (int)$price->value);
                    (int)$pay_history->balance = (int)$customer->temp_balance;
                    (int)$pay_history->user_id = (int)$tabinfo->user_id;
                    (int)$pay_history->passenger_id = (int)$passenger->id;
                    $pay_history->save();

                    $customer->save();
                } else {
                    (int)$pay_history->debit = (int) $price->value;
                    //(int)$customer->credit_limit = (int) ((int)$customer->credit_limit - (int)$price->value);
                    (int)$paid = (int)((int)$customer->temp_balance - (int)$price->value);
                    (int)$customer->temp_balance = (int)$paid;
                    (int)$pay_history->balance = (int)$paid;
                    (int)$pay_history->user_id = (int)$tabinfo->user_id;
                    (int)$pay_history->passenger_id = (int)$passenger->id;
                    $pay_history->save();
                    $customer->save();
    ...
    public function saveManualRequest(Request $request)
    {
        ...
        if ($tabinfo) {
            ...
            $tab->passenger_name = $passengers[0]['passenger_name'];
            $tab->pnr = $tabinfo['pnr'];
            $tab->user_id = $user->id;
            $tab->processed_by = auth()->user()->id;

            if ($tabinfo['transaction'] == 'TICKET') {
                $tab->tabtype_id = 1;
            } else if ($tabinfo['transaction'] == 'DATE-CHANGE') {
                $tab->tabtype_id = 4;
            } else if ($tabinfo['transaction'] == 'VOID') {
                $tab->tabtype_id = 3;
            } else if ($tabinfo['transaction'] == 'REFUND') {
                $tab->tabtype_id = 2;
            } else {
    ...
    public function ajaxAddPassengerOnUpdate(Request $request)
    {
        $passenger = collect($request->data['passenger']);
        ...
        $newPassenger = [
            ...
            'tabinfo_id' => $passenger['tab_id'],
            'processed_by' => Auth()->user()->id,

        ];
        $newPassenger = Passenger::create($newPassenger);
        $newPrice  = [
            'basic' => 0,
            'tax' => 0,
            'discount' => 0,
        ...
        $newPrice = Price::create($newPrice);
        $newVendor = Vendor::create($vendor);
        $passenger_tab = Tabinfo::findOrfail($newPassenger->tabinfo_id)->first();
        $customer = Customer::where('email', $passenger_tab->user->email)->first();
        if ((($passenger_tab->tabtype_id == 2) || ($passenger_tab->tabtype_id == 3) || ($passenger_tab->tabtype_id == 8) || ($passenger_tab->tabtype_id == 16)) && ($passenger_tab->status_id == 6)) {
            $paymenthistory = [
                'credit' => $newPrice->value,
                'debit' => 0,
                'balance' => (int)($customer->balance + (int)$newPrice->value),
                'user_id' => $passenger_tab->user_id,
                'passenger_id' => $newPassenger->id,
            ];

            $paymenthistory = PaymentHistory::create($paymenthistory);
        } else {
            $passengerhistory = [
                'credit' => 0,
                'debit' => $newPrice->value,
                'balance' => (int)($customer->balance - (int)$newPrice->value),
                'user_id' => $passenger_tab->user_id,
                'passenger_id' => $newPassenger->id,
            ];

            $paymenthistory = PaymentHistory::create($passengerhistory);
        }

        return response()->json(
            [
                'message' => 'Passenger Added on Update of this tab',
                'status' => 200
            ]
        );
    }
    ...
    {
        $passenger = $request->data['passenger'];

        if ($passenger) {
            //$passenger = Passenger::select('balance')->where('id', $passenger);
            //PaymentHistory::select()->where('passenger_id', $passenger);

            $passengerToDelete = Passenger::find($passenger);
            $customer = Customer::where('email', $passengerToDelete->tabinfo->user->email)->first();
            /*****
             *
             * $paid = 0;
                if ($tabinfo->tabtype_id == 2 || $tabinfo->tabtype_id == 3) {
                    (int)$pay_history->credit = (int)$price->value;
                    (int)$customer->credit_limit = (int)$customer->credit_limit + (int)$price->value;
                    (int)$customer->balance = (int)$customer->balance + (int)$price->value;
                    (int)$pay_history->balance = (int)$customer->balance;
                    (int)$pay_history->user_id = (int)$tabinfo->user_id;
                    (int)$pay_history->passenger_id = (int)$passenger->id;
                    $pay_history->save();
                    ...
                    (int)$pay_history->debit = (int) $price->value;
                    (int)$customer->credit_limit = (int) ((int)$customer->credit_limit - (int)$price->value);
                    (int)$paid = (int)((int)$customer->balance - (int)$price->value);
                    (int)$customer->balance = (int)$paid;
                    (int)$pay_history->balance = (int)$paid;
                    (int)$pay_history->user_id = (int)$tabinfo->user_id;
                    (int)$pay_history->passenger_id = (int)$passenger->id;
                    $pay_history->save();
             ...
             */
            $total_balance = 0;
            if ($passengerToDelete->tabinfo->status_id == 6) {
                if ($passengerToDelete->tabinfo->tabtype_id == 2 || $passengerToDelete->tabinfo->tabtype_id == 3 || $passengerToDelete->tabinfo->tabtype_id == 8 || $passengerToDelete->tabinfo->tabtype_id == 16) {

                    $total_balance = ((int)($customer->balance) - (int)$passengerToDelete->paymentHistory->credit);
                    $customer->balance = $total_balance;
                    $customer->save();
                }
                $total_balance = ((int)($customer->balance) + (int)$passengerToDelete->paymentHistory->debit);
                $customer->balance = $total_balance;
                $customer->save();
            }
            $passengerToDelete->delete();
            return response()->json(
                [
                    'message' => 'Passenger Deleted for this tab',
                    'status' => 200
                ]
            );
        } else {
            return response()->json(
                [
                    'message' => 'Error occur while deleting this tab',
                    'status' => 405
                ]
            );
        }
